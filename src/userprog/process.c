...
tid_t process_execute(const char *file_name, const char *args)
{
  ...
  // Combine file_name and args into one string if args is not NULL
  char *cmdline = NULL;
  if (args != NULL)
  {
    cmdline = malloc(strlen(file_name + 1) + strlen(args + 1) + 2);
    strlcpy(cmdline, file_name, strlen(file_name) + 1);
    strlcat(cmdline, " ", strlen(file_name) + strlen(args) + 2);
    strlcat(cmdline, args, strlen(file_name) + strlen(args) + 2);
  }
  else
  {
    cmdline = file_name;
  ...
  tid = thread_create(strtok_r(cmdline, " ", &save_ptr), PRI_DEFAULT, start_process, fn_copy);
  if (tid == TID_ERROR)
...
start_process(void *file_name_)
{
  ...
  // Check if there is any argument apart from the file name
  if (1)
  {
    *(file_name + strlen(file_name)) = ' ';
    if (save_ptr == file_name + strlen(file_name) - 1)
      *(save_ptr) = '\0';
...
void process_exit(void)
{
  ...
  // not needed due to system call implementation
